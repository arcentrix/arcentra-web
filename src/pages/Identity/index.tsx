...
export default function IdentityPage() {
  ...
  const handleEdit = async (provider: IdentityProvider) => {
    try {
      const fullProvider = await Apis.identity_integration.getIdentityProvider(
        provider.name,
      );
  ...
  const handleSubmit = async (data: CreateIdentityProviderRequest) => {
    if (selectedProvider) {
      ...
      await Apis.identity_integration.updateIdentityProvider(
  ...
  const handleToggle = async (provider: IdentityProvider) => {
    try {
      ...
      toast.success(
        provider.isEnabled === 1 ? "Provider disabled" : "Provider enabled",
      );
      await loadProviders();
    } catch (error) {
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">
              Manage authentication, SSO and identity providers for the
              workspace.
            </p>
          </div>
          {activeTab === "providers" && (
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {providers.length - enabledCount} disabled
              </p>
            </CardContent>
          </Card>
          <Card>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Configure and manage supported identity providers (OAuth,
                      LDAP, OIDC, SAML)
                    </CardDescription>
                  </div>
                </div>
