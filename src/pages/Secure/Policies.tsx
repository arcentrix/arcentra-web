...
import { CheckCircle2, GitMerge, Lock, ShieldCheck, ShieldX, type LucideIcon } from 'lucide-react'
...
const POLICIES: Policy[] = [
  {
    ...
    description: 'At least 1 approver from the maintainers team must approve before promote.',
    status: 'active',
  ...
  },
  {
    ...
    description: 'Pipelines using `public-cloud` agent pool cannot access secret scope `prod:*`.',
    status: 'active',
  ...
  },
  {
    ...
    description: 'Members with the Admin role must have MFA enabled within 24 hours.',
    status: 'warning',
  ...
  },
  {
    ...
    description: 'Agents registered outside the cluster require tokens limited to `build:*`.',
    status: 'active',
    ...
    <Tabs defaultValue='access' className='w-full'>
      ...
      <TabsContent value='access' className='mt-4'>
        <div className='grid gap-3'>
          {POLICIES.map((policy) => (
            <Card key={policy.id} className='transition-colors hover:bg-accent/30'>
