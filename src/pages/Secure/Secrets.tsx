    ...
    <Tabs defaultValue='all' className='w-full'>
      ...
      <TabsContent value='all' className='mt-4'>
        <div className='grid gap-3'>
          {SECRETS.map((secret) => (
            <Card key={secret.id} className='transition-colors hover:bg-accent/30'>
              <CardContent className='flex items-center gap-4 p-4'>
                ...
                <div className='min-w-0 flex-1'>
                  ...
                  <div className='mt-0.5 text-xs text-muted-foreground'>
                    {secret.source} · used by {secret.consumers} pipeline{secret.consumers === 1 ? '' : 's'}
                  </div>
                </div>
                <div className='hidden text-right text-xs text-muted-foreground sm:block'>
                  <div className='font-medium text-foreground'>{secret.expires}</div>
      ...
      <TabsContent value='providers' className='mt-4'>
        <div className='grid gap-3 md:grid-cols-2'>
          {[
            ...
            { name: 'External Secrets Operator', status: 'connected', secrets: 5 },
          ].map((provider) => (
