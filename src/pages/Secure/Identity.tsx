...
function userStatusBadge(user: User) {
  if (user.invitationStatus === 'pending') return <Badge variant='warning'>Invited</Badge>
  if (user.invitationStatus === 'expired') return <Badge variant='secondary'>Expired</Badge>
  if (user.invitationStatus === 'revoked') return <Badge variant='destructive'>Revoked</Badge>
...
function UsersPanel() {
  ...
  const handleToggleStatus = async (user: User) => {
    ...
    // optimistic update
    setUsers((prev) =>
      prev.map((u) => (u.userId === user.userId ? { ...u, isEnabled: next } : u)),
    )
    try {
                ...
                <TableCell>
                  <div className='flex items-center gap-3'>
                    <Avatar className='h-8 w-8'>
                      <AvatarImage alt={displayName} src={user.avatar || DEFAULT_USER_AVATAR} />
                      <AvatarFallback className='text-[10px]'>{getInitials(user)}</AvatarFallback>
                    </Avatar>
                    <div className='min-w-0'>
                      <div className='line-clamp-1 font-medium'>{displayName}</div>
