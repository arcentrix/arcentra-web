...
export default function SystemInfoPage() {
  ...
  const getPluginTypeColor = (type: PluginType) => {
    const colors: Record<PluginType, string> = {
      ...
      deploy: 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300',
      security: 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300',
      notify: 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300',
      approval: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300',
      ...
      <div className="flex-1 space-y-6 p-4 md:p-8 pt-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-8 text-muted-foreground">Loading system information...</div>
          </CardContent>
        </Card>
      </div>
    );
        ...
        <CardContent className="space-y-4">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="h-9">
              ...
              <TabsTrigger value="source" className="text-xs px-3">Source</TabsTrigger>
              <TabsTrigger value="custom" className="text-xs px-3">Custom</TabsTrigger>
              <TabsTrigger value="notify" className="text-xs px-3">Notify</TabsTrigger>
            </TabsList>
      ...
      <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
        <SheetContent side="right" className="w-full sm:max-w-lg">
          <SheetHeader>
            ...
            <SheetDescription>Plugin details and configuration</SheetDescription>
          </SheetHeader>
          {selectedPlugin && (
            <div className="mt-6 space-y-6">
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm font-medium mt-1">{selectedPlugin.version}</p>
                </div>
                <div>
                ...
                {selectedPlugin.description && (
                  <div>
                    ...
                    <p className="text-sm mt-1 text-muted-foreground">{selectedPlugin.description}</p>
                  </div>
                )}
