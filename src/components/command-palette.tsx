...
const PAGE_ITEMS: Omit<CommandItem, 'group'>[] = [
  { id: 'overview', title: 'Overview', description: 'Dashboard', icon: LayoutDashboard, to: '/' },
  { id: 'build', title: 'Build', description: 'Pipelines · Runs · Artifacts', icon: Hammer, to: '/build' },
  { id: 'deploy', title: 'Deploy', description: 'Deployments · Environments · Releases', icon: Rocket, to: '/deploy' },
  { id: 'agents', title: 'Agents', description: 'Runtime · Marketplace · Runners', icon: Bot, to: '/agents' },
  { id: 'observe', title: 'Observe', description: 'Logs · Metrics · Traces · Audit', icon: Activity, to: '/observe' },
  { id: 'secure', title: 'Secure', description: 'Access · Secrets · Policies', icon: Shield, to: '/secure' },
  { id: 'settings', title: 'Settings', description: 'Workspace · Integrations · Billing', icon: Settings2, to: '/settings' },
]
...
export function CommandPalette() {
  ...
  const filtered = useMemo(() => ALL_ITEMS.filter((i) => matches(i, query)), [query])
    ...
    <Dialog open={paletteOpen} onOpenChange={uiShellStore.setPaletteOpen}>
      <DialogPortal>
        ...
        <DialogPrimitive.Content
          className='fixed left-1/2 top-[18vh] z-50 w-full max-w-2xl -translate-x-1/2 overflow-hidden rounded-lg border bg-background shadow-2xl duration-150 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:zoom-in-95 data-[state=closed]:zoom-out-95'
        >
        ...
        <ScrollArea className='max-h-[400px]'>
          <div className='p-2'>
            ...
            {(['Pages', 'Actions', 'Documentation'] as const).map((groupName) => {
