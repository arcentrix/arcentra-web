      ...
      try {
        ...
        if (!cookieToken) {
          for (const [name, value] of Object.entries(allCookies)) {
            ...
            if (
              (lowerName.includes("token") || lowerName.includes("auth")) &&
              value
            ) {
              cookieToken = value;
              if (import.meta.env.DEV) {
        ...
        if (cookieToken) {
          ...
          if (import.meta.env.DEV) {
            console.log(
              "Token set to authStore, accessToken length:",
              cookieToken.length,
            );
          }
        } else {
          if (import.meta.env.DEV) {
            console.log(
              "No token found in cookies, available cookies:",
        ...
        try {
          ...
          if (!cookieToken) {
            try {
              ...
              if (import.meta.env.DEV) {
                console.log(
                  "Failed to get token from refreshToken API:",
                  tokenError,
                );
              }
        ...
        if (userInfo) {
          ...
          if (!currentAuthState.accessToken) {
            if (import.meta.env.DEV) {
              console.log(
                "User info exists but no token, backend uses cookie authentication",
              );
            }
            // 设置一个标记，表示认证已通过
