...
import type { AuthMethod } from '@/api/auth/types'

const authMethodLabels: Record<AuthMethod, string> = {
  standard: 'Native (Username/Password)',
  ldap: 'LDAP',
  oauth2: 'OAuth 2.0',
...
export function LoginForm({
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      if (authMethod === 'ldap') {
        response = await Apis.auth.loginWithLDAP('ldap', {
          username: data.username,
          password: data.password,
        })
      } else {
        response = await Apis.auth.login({
          username: data.username,
          password: data.password,
          authMethod,
        })
      ...
      <FieldGroup>
        <div className='flex flex-col items-center gap-1 text-center'>
          ...
          <p className='text-muted-foreground text-sm text-balance'>
            Choose your authentication method and login
          </p>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button 
                ...
                type='button'
              >
                {authMethodLabels[authMethod]}
                <ChevronDown className='ml-2 h-4 w-4 opacity-50' />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className='w-[var(--radix-dropdown-menu-trigger-width)]' align='start'>
              <DropdownMenuItem onClick={() => setAuthMethod('standard')}>
                Native (Username/Password)
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setAuthMethod('ldap')}>
                LDAP
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setAuthMethod('oauth2')}>
                OAuth 2.0
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setAuthMethod('oidc')}>
                OpenID Connect (OIDC)
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <>
            <Field>
              <FieldDescription className='text-center mb-4'>
                Select an OAuth2 provider to continue
              </FieldDescription>
              <div className='grid gap-3'>
                ...
                <Button
                  ...
                  disabled={isLoading}
                  className='w-full'
                  onClick={() => handleOAuth2Login('slack')}
                >
                  <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' className='mr-2 h-4 w-4'>
                    <path
